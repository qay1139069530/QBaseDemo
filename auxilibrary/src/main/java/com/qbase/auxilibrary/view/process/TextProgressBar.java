...
public class TextProgressBar extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mProgressBarHeight = (int) ta.getDimension(R.styleable.TextProgressBar_progress_barHeight, 2);

        mProgressColor = ta.getColor(R.styleable.TextProgressBar_progressColor, ContextCompat.getColor(context, R.color.colorPrimary));
        mProgressBackgroundColor = ta.getColor(R.styleable.TextProgressBar_backgroundColor, Color.WHITE);
        mTxtColor = ta.getColor(R.styleable.TextProgressBar_textColor, ContextCompat.getColor(context, R.color.colorPrimary));
        ta.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRect(0, getHeight() - mProgressBarHeight, getWidth(),getHeight(), mPaint);
        ...
        // 显示文字在进度条之上
        int x = getProgressWidth() - mRect.centerX();
        int y = (getHeight() - mProgressBarHeight) / 2 - mRect.centerY();
        mPaint.setColor(mTxtColor);
        canvas.drawText(mTxt, x, y, mPaint);
        // 绘制进度线
        mPaint.setColor(mProgressColor);
        canvas.drawRect(0, getHeight() - mProgressBarHeight,getProgressWidth(), getHeight(), mPaint);
    }
