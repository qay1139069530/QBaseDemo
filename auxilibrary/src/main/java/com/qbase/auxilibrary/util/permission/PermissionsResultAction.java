...
/**
 * This abstract class should be used to create an if/else action that the PermissionsManager2
...
public abstract class PermissionsResultAction {
    ...
    /**
     * Alternate Constructor. Pass the looper you wish the PermissionsResultAction2
     ...
     * affect whether the PermissionsResultAction2 should continue
     ...
     *                   Android version
     * @return return true if the PermissionsResultAction2 should
     * ignore the lack of the permission and proceed with exection
     * or false if the PermissionsResultAction2 should treat the
    ...
    protected synchronized final boolean onResult(final @NonNull String permission, int result) {
        if (result == PackageManager.PERMISSION_GRANTED) {
            return onResult(permission, PermissionsEnum.GRANTED);
        } else {
            return onResult(permission, PermissionsEnum.DENIED);
        }
    ...
    @SuppressWarnings("WeakerAccess")
    @CallSuper
    protected synchronized final boolean onResult(final @NonNull String permission, PermissionsEnum result) {
        mPermissions.remove(permission);
        if (result == PermissionsEnum.GRANTED) {
        ...
        } else if (result == PermissionsEnum.DENIED) {
            new Handler(mLooper).post(new Runnable() {
                @Override
                public void run() {
                    onDenied(permission);
            ...
            return true;
        } else if (result == PermissionsEnum.NOT_FOUND) {
    ...
    /**
     * This method registers the PermissionsResultAction2 object for the specified permissions
     * so that it will know which permissions to look for changes to. The PermissionsResultAction2
